...
document.addEventListener('DOMContentLoaded', () => {
    ...
    UIManager.elements.processBtn.addEventListener('click', async () => {
        ...
        const divisionMode = UIManager.elements.divisionMode.value;
        const flipMode = UIManager.elements.flipMode.value;

        let signatureSize;
        if (divisionMode === 'single') {
            // El tamaño de la firma es el total de páginas redondeado al siguiente múltiplo de 4
            signatureSize = Math.ceil(totalPages / 4) * 4;
        } else {
            signatureSize = parseInt(UIManager.elements.signatureSize.value);
        }
