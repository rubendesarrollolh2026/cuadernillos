...
const UIManager = {
    elements: {
        ...
        divisionMode: document.getElementById('division-mode'),
        signatureSize: document.getElementById('signature-size'),
        signatureSizeContainer: document.getElementById('signature-size-container'),
    ...
    init() {
        ...
        this.elements.divisionMode.onchange = () => {
            const isSingle = this.elements.divisionMode.value === 'single';
            this.elements.signatureSizeContainer.classList.toggle('hidden', isSingle);
            // Disparar actualización de previsión
            const totalPages = parseInt(this.elements.totalPages.textContent);
            if (!isNaN(totalPages)) {
                this.updatePrediction(totalPages, parseInt(this.elements.signatureSize.value));
            }
        };
    ...
    updatePrediction(totalPages, signatureSize) {
        ...
        if (this.elements.divisionMode.value === 'single') {
            this.elements.bookletsCount.textContent = "1";
            const needed = Math.ceil(totalPages / 4) * 4;
            const blanks = needed - totalPages;
            if (blanks > 0) {
                this.elements.bookletsCount.innerHTML = `1 <small style="display:block; font-weight:normal; color:var(--text-secondary); margin-top:4px;">(Se añadirán ${blanks} página(s) en blanco para completar el cuadernillo de ${needed})</small>`;
            }
        } else {
